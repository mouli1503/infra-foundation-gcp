# ── Secret Manager ───────────────────────────────────────────────────────────
# Secrets for this project (internal-apps-staging).
#
...
# (internal-apps-staging-tfstate), encrypted at rest — keep that bucket locked down.
#
# To add another service's secrets, just add more entries to `secrets` in
# secrets.auto.tfvars (one secret_id => value per line).
