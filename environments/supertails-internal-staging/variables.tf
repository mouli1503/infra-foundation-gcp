...
variable "domain" {
  type    = string
  default = "apps.staging.supertails.com"
}

variable "routes" {
  description = "Map of route_key (subdomain) -> Cloud Run service name. e.g. hello -> hello.apps.staging.supertails.com"
  type        = map(string)
}
...
variable "secret_accessors" {
  description = "Default members granted roles/secretmanager.secretAccessor on every secret (e.g. the Cloud Run runtime service account, serviceAccount:...@internal-apps-staging.iam.gserviceaccount.com)."
  type        = set(string)
  default     = []
}
...
variable "route_host_overrides" {
  description = "Optional per-route explicit host (FQDN) overrides. route_key -> host. Use when the desired host doesn't fit the {route_key}.{domain} pattern (e.g. multi-label subdomains). Resource names still derive from route_key, which must be dot-free."
  type        = map(string)
  default     = {}
}
...
variable "iap_callback_route" {
  description = "Route key that receives GET / (IAP callback). Must be an IAP-protected route so IAP validates the token and redirects back to the app; that backend must return 200 for GET /."
  type        = string
  default     = null
}
