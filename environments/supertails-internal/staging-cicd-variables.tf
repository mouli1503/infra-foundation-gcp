...
variable "workload_identity_pool_id" {
  description = "Existing Workload Identity Pool ID (do not include full resource path)."
  type        = string
  default     = "github-actions"

  validation {
    condition     = !var.enable_staging_cicd || trimspace(var.workload_identity_pool_id) != ""
    error_message = "workload_identity_pool_id is required when enable_staging_cicd is true."
  }
}

variable "workload_identity_provider_id" {
  description = "Existing Workload Identity Provider ID (do not include full resource path)."
  type        = string
  default     = "github-oidc"

  validation {
    condition     = !var.enable_staging_cicd || trimspace(var.workload_identity_provider_id) != ""
    error_message = "workload_identity_provider_id is required when enable_staging_cicd is true."
  }
}
